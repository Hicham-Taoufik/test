 // --- Configuration & Constants ---
         ...
         printWindow.document.write(`
            ...
            <script>
                window.onload = function() {
                    // Use timeout to ensure image might be loaded
                    setTimeout(function() {
                        window.print();
                        // Close after print dialog is likely handled
                        setTimeout(function() { window.close(); }, 500);
                    }, 500); // Delay printing slightly
                }
            <\/script>
            </body></html>
         `);
      ...
      // ... (startIdCapture, stopIdCapture, takePhotoAndExtract, autofillCreateForm functions remain the same as previous version) ...
        ...
        async function startIdCapture() {
          if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            ...
            DOM.frontPreview.style.display = 'none'; DOM.frontPreview.src = '';
            DOM.backPreview.style.display = 'none'; DOM.backPreview.src = '';
            ...
            } catch (err) { updateCaptureMessage(`Erreur caméra: ${err.message}`, 'error'); stopIdCapture(false); }
          } else { alert("Accès caméra non supporté."); updateCaptureMessage("Caméra non supportée.", 'error'); DOM.captureIdButton.disabled = false; }
        }
        function stopIdCapture(clearBlobs = true) {
          if (idCaptureStream) { idCaptureStream.getTracks().forEach(track => track.stop()); }
          ...
          if (clearBlobs) { frontImageBlob = null; backImageBlob = null; DOM.frontPreview.src = ''; DOM.backPreview.src = ''; DOM.frontPreview.style.display = 'none'; DOM.backPreview.style.display = 'none'; console.log("ID Capture cancelled and blobs cleared."); }
        }
       async function takePhotoAndExtract() {
            if (!idCaptureStream || !DOM.idVideo || DOM.idVideo.readyState < 2) { updateCaptureMessage('Caméra non prête.', 'warning'); return; }
            updateCaptureMessage('Capture et analyse...', 'loading');
            DOM.takePhotoButton.disabled = true; DOM.cancelCaptureButton.disabled = true;
            const canvas = DOM.idCanvas; canvas.width = DOM.idVideo.videoWidth; canvas.height = DOM.idVideo.videoHeight;
            const context = canvas.getContext('2d'); context.drawImage(DOM.idVideo, 0, 0, canvas.width, canvas.height);
            canvas.toBlob(async (blob) => {
                if (!blob) { updateCaptureMessage('Erreur capture image.', 'error'); stopIdCapture(); return; }
                if (isCapturingFront) {
                    frontImageBlob = blob; console.log("Recto captured"); DOM.frontPreview.src = URL.createObjectURL(frontImageBlob); DOM.frontPreview.style.display = 'inline-block'; isCapturingFront = false; DOM.captureInstruction.textContent = "Positionnez le VERSO et prenez la photo."; updateCaptureMessage('Recto OK. Préparez le verso.', 'info'); DOM.takePhotoButton.disabled = false; DOM.cancelCaptureButton.disabled = false;
                } else {
                    backImageBlob = blob; console.log("Verso captured"); DOM.backPreview.src = URL.createObjectURL(backImageBlob); DOM.backPreview.style.display = 'inline-block';
                    ...
                    if (frontImageBlob && backImageBlob) {
                        ...
                        formData.append('data', frontImageBlob, 'id_card_front.jpg'); // N8N binary data often expects 'data' key
                        formData.append('data', backImageBlob, 'id_card_back.jpg');
                         try {
                            ...
                            // Assuming N8N returns { message: ..., data: { extracted JSON } }
                             if (extractedData && extractedData.data) {
                                autofillCreateForm(extractedData); showToast('Formulaire pré-rempli!', 'success'); updateCaptureMessage('Données extraites!', 'success');
                             } else { const errorMessage = extractedData?.message || "Aucune donnée extraite."; throw new Error(errorMessage); }
                         } catch (error) { console.error("Error extraction API call:", error); updateCaptureMessage(`Erreur extraction: ${error.message}`, 'error');
                         } finally { frontImageBlob = null; backImageBlob = null; /* Clear blobs after attempt */ }
                    } else { console.error("Missing blobs."); updateCaptureMessage('Erreur: Images manquantes.', 'error'); }
                }
            }, 'image/jpeg', 0.9);
        }
        function autofillCreateForm(data) {
             const form = DOM.createForm;
             if (!form || !data || !data.data) { console.warn("Autofill failed: Form or data object missing."); return; }
             const extracted = data.data; console.log("Extracted from API for Autofill:", extracted);
             let formattedDateOfBirth = ""; if (extracted.date_of_birth) { const originalDateString = extracted.date_of_birth; const dateParts = originalDateString.split('/'); if (dateParts.length === 3) { const day = dateParts[0]; const month = dateParts[1]; const year = dateParts[2]; formattedDateOfBirth = `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`; console.log("Formatted date_of_birth:", formattedDateOfBirth); } else { console.warn("DOB format unexpected:", originalDateString); formattedDateOfBirth = ""; } } else { console.warn("DOB missing."); }
             form.nom.value = extracted.last_name ?? ''; form.prenom.value = extracted.first_name ?? ''; form.cin.value = extracted.id_number ?? ''; form.date_naissance.value = formattedDateOfBirth; form.adresse.value = extracted.address ?? ''; form.ville.value = extracted.city ?? ''; form.sexe.value = extracted.gender === 'F' ? 'F' : (extracted.gender === 'M' ? 'M' : ''); // Handle potential empty gender
             form.querySelectorAll('.input-group.has-error').forEach(el => el.classList.remove('has-error')); form.querySelectorAll('input, select').forEach(el => el.classList.remove('input-error')); form.querySelectorAll('.error-text').forEach(el => el.textContent = '');
             showMessage('message', 'Formulaire pré-rempli. Vérifiez les informations.', 'info');
              // Trigger change for select2 fields if needed
      ...
      async function handlePatientSearch() {
        ...
        if (!cin) {
          ...
          DOM.getResult.style.display = 'block'; // Ensure area is visible for message
          DOM.getResult.innerHTML = '<div class="patient-result-container"><p class="message message-warning">Veuillez entrer un CIN.</p></div>';
          return;
        }
        ...
        try {
          ...
          // CORRECTED CHECK: Check if it's an array and has at least one element
          if (patientResponse && Array.isArray(patientResponse) && patientResponse.length > 0) {
            const currentPatientInfo = patientResponse[0]; // Get the first patient object
            currentIPP = currentPatientInfo.ipp; // Get IPP from the fetched data
            ...
            try {
              if (currentIPP) {
                console.log(`Calling startVisit for existing patient IPP: ${currentIPP}`);
            ...
            if (currentPatientInfo.date_naissance) {
              // Assuming date comes as YYYY-MM-DD from DB
              const dateParts = currentPatientInfo.date_naissance.split("-");
              if (dateParts.length === 3) {
                displayDate = `${dateParts[2]}/${dateParts[1]}/${dateParts[0]}`; // Format as DD/MM/YYYY
              } else {
            ...
            DOM.resultDiv.innerHTML = qrCodeHTML; // Display results

          } else {
            // Patient not found
            showMessage("getResult", "", ""); // Clear loading message
            DOM.resultDiv.innerHTML = '<div class="patient-result-container"><p class="message message-warning">Patient non trouvé pour ce CIN.</p></div>';
          }
        } catch (error) {
          console.error("Search Patient Process Error:", error);
          showMessage("getResult", "", ""); // Clear loading message
          DOM.resultDiv.innerHTML = `<div class="patient-result-container"><p class="message message-error">Erreur lors de la recherche: ${error.message}</p></div>`;
        }
      }
      ...
      async function handleCreatePatient(event) {
        ...
        const payload = {
          ...
          cin: DOM.createForm.cin.value.trim() || null, // Send null if empty
          ...
          sexe: DOM.createForm.sexe.value,
           // Send boolean for has_insurance based on mutuelle selection
           has_insurance: !!DOM.mutuelleInput.value, // true if a mutuelle is selected, false otherwise
          mutuelle: DOM.mutuelleInput.value.trim() || null, // Send null if empty
          doctor: DOM.doctorInput.value || null // Send selected doctor matricule or null
        };
        console.log("Payload for Create Patient:", payload);

        try {
          // Call create patient endpoint (which now also starts the visit)
          const createResponse = await apiService.createPatient(payload);
          console.log("Create Patient API Response:", createResponse);

           // N8N workflow now returns IPP and visit_id directly on success
           if (createResponse && createResponse.success && createResponse.ipp && createResponse.visit_id !== undefined) {
              currentIPP = createResponse.ipp; // Store the new IPP
              ...
              const qrCodeData = generateQrData(currentIPP);
              if (qrCodeData) {
                  showMessage('createResult', `Patient créé (IPP: ${sanitizeInput(currentIPP)})`, 'result'); // Use specific 'result' type for display logic
                  DOM.createResultMessage.textContent = `Patient créé (IPP: ${sanitizeInput(currentIPP)}) - Visite ID: ${createResponse.visit_id}`; // Display IPP and Visit ID
                  ...
                  DOM.createResultDiv.style.display = 'block'; // Show the result div
              } else {
                  // Show success message even if QR fails
              ...
              $(DOM.mutuelleInput).val(null).trigger('change'); // Reset Select2
              $(DOM.doctorInput).val(null).trigger('change');   // Reset Select2

          } else {
              // Handle cases where N8N might return success: false or missing data
            ...
            // Display error in the main message area
            showMessage('message', `Erreur: ${apiError.message}`, 'error');
            DOM.createResultDiv.style.display = 'none'; // Ensure QR area is hidden on error
        } finally {
      ...
      DOM.cinInput?.addEventListener("keypress", (e) => { if (e.key === "Enter") { e.preventDefault(); handlePatientSearch(); } }); // Added preventDefault
      ...
      function initializePage() {
            ...
            // Add listeners for session timeout reset
      ...
      // --- Initialize Dropdowns ---
       async function fetchMutuelles() {
           try {
               const data = await apiService.fetchMutuelles(); // Use apiService
               if (data.success && Array.isArray(data.mutuelles)) {
                   populateMutuelleDropdown(data.mutuelles);
               } else { console.error('Failed to fetch mutuelles:', data); }
       ...
       function populateMutuelleDropdown(mutuelles) {
           ...
           if (!dropdown) return;
           dropdown.innerHTML = '<option value="" selected>Aucune / Non spécifié</option>'; // Default empty option
           mutuelles.forEach(mutuelleName => { // Assuming it's an array of names
       ...
       async function fetchDoctors() {
           try {
               const data = await apiService.fetchDoctors(); // Use apiService
               if (data.success && Array.isArray(data.doctors)) {
                   populateDoctorsDropdown(data.doctors);
               } else { console.error('Failed to fetch doctors:', data); }
           } catch (error) { console.error('Error fetching doctors:', error); }
       }
       function populateDoctorsDropdown(doctors) {
            const dropdown = DOM.doctorInput;
            if (!dropdown) return;
            dropdown.innerHTML = '<option value="" selected>Choisir un médecin...</option>'; // Default empty option
       ...
       // --- Run Initialization ---
       // Use DOMContentLoaded to ensure elements are ready before JS runs
