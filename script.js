(() => {
  ...
  const DOM = {
    ...
    // Updated/New buttons:
    createPrintQrButton: document.getElementById("createPrintQrButton"), // Renamed - now the only print button
  ...
  const showMessage = (elementId, message, type = "info") => {
    ...
    if (elementId === "createResult") {
        ...
        const btnContainer = el.querySelector(".print-buttons-container"); // Still keep container for layout if needed
        ...
        if (img) {
         ...
         if (btnContainer) {
             ...
             btnContainer.style.display = isResult ? "flex" : "none"; // Keep showing/hiding container
         }
  ...
  const printQRCode = (qrImageUrl) => {
    ...
    // --- ADDED LOGGING OF QR IMAGE URL RIGHT BEFORE PRINT WINDOW WRITE ---
    console.log("printQRCode: QR Image URL about to be written to print window:", qrImageUrl);

    // --- SIMPLIFIED PRINT WINDOW CONTENT FOR TESTING ---
    printWindow.document.write(`
      ...
      <body>
        <img id="qrCodeImageToPrint" src="${qrImageUrl}" alt="QR Code Patient" onload="setTimeout(window.print, 100);" onerror="console.error('QR Code image failed to load for printing');" />
        <script>
          // Optional error handling in case image fails to load
          document.getElementById('qrCodeImageToPrint').onerror = function() {
              console.error("Error loading QR Code image in print window:", "${qrImageUrl}");
          };
        <\/script>
      </body>
      </html>
  ...
  const startIdCapture = async () => {
     // ... (startIdCapture function remains the same)
  };

  const stopIdCapture = (clearBlobs = true) => {
     // ... (stopIdCapture function remains the same)
  };

  const takePhotoAndExtract = async () => {
     // ... (takePhotoAndExtract function remains the same)
  };

  const autofillCreateForm = (data) => {
     // ... (autofillCreateForm function remains the same)
  };

  // --- Form Validation ---
  const validateField = (input, validationFn, errorMessage) => {
     // ... (validateField function remains the same)
  };

  const validateCreateForm = () => {
     // ... (validateCreateForm function remains the same)
  };

  // --- Event Handlers ---
  const handlePatientSearch = async () => {
     // ... (handlePatientSearch function remains the same)
  };
  ...
  const handleCreatePatient = async (event) => {
    ...
    DOM.createPrintQrButton.disabled = true;   // Ensure button starts disabled
    ...
    try {
      ...
      if (createResponse && (createResponse.success === true || String(createResponse.success).toLowerCase() === 'true') && createResponse.ipp) {
        ...
        if (qrCodeData && qrCodeData.qrImageUrl) {
          ...
          // Enable QR Print button and assign handler
